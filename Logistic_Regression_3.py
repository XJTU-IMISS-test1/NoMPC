...
class LogisticRegression:
    ...
    def __init__(self, weight_vector, batch_size, max_iter, alpha,
                 ...
                 sigmoid_func=None, logname=None):
        """
        构造函数:初始化
        """
    ...
    def fit_model_distributed_input(self, X_trainA, X_trainB, Y_train, instances_count, indice_littleside):
        ...
        while self.n_iteration <= self.max_iter:
            ...
            if (np.isnan(loss) or np.isinf(loss)):
                # file.write("\nEpoch num: {}, last epoch loss: {}".format(self.n_iteration, loss_decrypt))
                file_name = os.path.join(self.logname,
                                         "alpha={}_batch_size={}_max_iter={}_lambda_para={}_C={}_sigmoid_func={}.txt".format(
                                             self.alpha, self.batch_size, self.max_iter, self.lambda_para, self.c,
                                             self.sigmoid_func))
    ...
    def OVRClassifier(self, X_train1, X_train2, X_test1, X_test2, Y_train, Y_test):
        ...
        for i in range(len(label_lst)):
            ...
            self.OVRModel_Agg.update({
                                         pos_label: self.OVRModel_X})  # 存储 ############################################################################
...
def read_distributed_data_raw_or_sketch(dataset_name, raw_or_sketch, kernel_method, portion, sampling_k, ovr,
    ...
    if raw_or_sketch == "sketch" and kernel_method in ["pminhash", "0bitcws"]:
        ...
        X_train1 = np.loadtxt(os.path.join(main_path, dataset_file_name, "portion37_" + kernel_method, "sketch" + str(sampling_k), "X1_train.txt"), delimiter=",")
        X_train2 = np.loadtxt(os.path.join(main_path, dataset_file_name, "portion37_" + kernel_method, "sketch" + str(sampling_k), "X2_train.txt"), delimiter=",")
        # Y_train = np.loadtxt(os.path.join(main_path, dataset_file_name, "portion37_" + kernel_method, "sketch" + str(sampling_k), "X1_train.txt"))
        X_test1 = np.loadtxt(os.path.join(main_path, dataset_file_name, "portion37_" + kernel_method, "sketch" + str(sampling_k), "X1_test.txt"), delimiter=",")
        X_test2 = np.loadtxt(os.path.join(main_path, dataset_file_name, "portion37_" + kernel_method, "sketch" + str(sampling_k), "X2_test.txt"), delimiter=",")
        # Y_test = np.loadtxt(os.path.join(main_path, dataset_file_name, "portion37_" + kernel_method, "sketch" + str(sampling_k), "X1_train.txt"))
        # gamma_scale =
...
def No_MPC_Model(weight_vector, batch_size, max_iter, alpha, eps, ratio, penalty, lambda_para, data_tag, C, multi_class,
    ...
    LogisticRegressionModel = LogisticRegression(weight_vector=weight_vector, batch_size=batch_size,
                                                 ...
                                                 Epoch_list_max=Epoch_list_max, sigmoid_func=sigmoid_func,
                                                 logname=logname)
...
if __name__ == "__main__":
    ...
    ##################数据集参数####################
    dataset_name = "webspam10k"  # cifar10
    ...
    multi_class = 'bin'  # 二分类bin还是多分类ovr
    ...
    # 数据集训练前的处理
    X_train1, X_train2, Y_train, X_test1, X_test2, Y_test = read_distributed_data_raw_or_sketch(
    ...
    # batch_size_lst = [128, 16]
    # alpha_lst = [0.00001, 0.0001, 0.001, 0.01, 0.1, 1]
    # lambda_para_lst = [0, 0.1, 1]
    batch_size_lst = [16, 32, 64, 128, 256, 512]
    alpha_lst = [0.0001, 0.001, 0.01]
    lambda_para_lst = [1]


    # C_lst = [0.001, 0.01, 0.1, 1, 10, 100]
    C_lst = [0.01, 0.1, 1, 5, 10, 50, 100]
    sigmoid_func_lst = ["linear", "origin"]  # ["linear", "cube", "segmentation", "origin"]
        ...
        for batch_size in batch_size_lst:
            for alpha in alpha_lst:
                for lambda_para in lambda_para_lst:
                    for C in C_lst:
                        No_MPC_Model(weight_vector, batch_size, max_iter, alpha, eps, ratio, penalty, lambda_para,
                                     data_tag, C, multi_class, X_train1, X_train2, X_test1, X_test2, Y_train, Y_test,
                                     Epoch_list_max, sigmoid_func, log_path)
